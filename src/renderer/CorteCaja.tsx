      ...
      <tbody>
        {boletos.map((b) => (
          <tr key={b.id} style={{ borderBottom: '1px solid #eee' }}>
            <td>{formatearFolioPlano(b.serie, b.folio)}</td>
...
export function CorteCaja({
  ...
  function datosReporteGeneral(): DatosReporteCrudo | undefined {
    if (!detalleActual) return undefined
    return {
      variante: 'general',
      datos: {
        ...
        gastosDelPeriodo: detalleActual.gastosDelPeriodo.map((g) => ({ concepto: g.concepto, monto: g.monto })),
  ...
  function datosReporteSerie(s: DetalleCortePorSerie): DatosReporteCrudo {
    return {
      variante: 'serie',
      datos: {
        estacionamientoNombre: nombreEstacionamiento,
        ...
        boletos: s.boletos.map((b) => ({
          serie: b.serie,
          folio: b.folio,
          tipoVehiculo: b.tipoVehiculo,
          horaEntrada: b.horaEntrada,
          horaSalida: b.horaSalida,
          monto: b.monto
        })),
        totalBoletos: s.totalBoletos,
        totalMonto: s.totalMonto
      }
    }
  }

  async function imprimirElemento(id: string, datosReporte?: DatosReporteCrudo): Promise<void> {
    const elemento = document.getElementById(id)
    if (!elemento) return
    try {
      await window.api.imprimir({ html: elemento.outerHTML, tipo: 'reporte', datosReporte })
    } catch (e) {
      setError(String(e))
    }
  }
        ...
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', margin: '0.5rem 0' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <button onClick={() => imprimirElemento('corte-general-imprimible', datosReporteGeneral())}>
                Reimprimir corte general
              </button>
              ...
              {detalleActual.porSerie.map((s) => (
                <div key={s.serie} style={{ marginBottom: '1.5rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <button onClick={() => imprimirElemento(`corte-serie-${s.serie}-imprimible`, datosReporteSerie(s))}>
